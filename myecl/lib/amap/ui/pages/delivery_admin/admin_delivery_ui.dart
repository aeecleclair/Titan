...
class DeliveryAdminUi extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return deliveryOrderList.when(
      data: (deliveries) {
        if (deliveries.isNotEmpty) {
          final orderList =
              deliveries[c] ?? const Tuple2(AsyncValue.loading(), false);
          final Map<Product, int> productQuantityDict = {};
          orderList.item1.when(
            data: (orders) {
              ...
              children: [
                Row(
                  children: [
                    ...
                    GestureDetector(
                        child: orderList.item1.when(
                          data: (items) {
                            return Container(
                                padding: const EdgeInsets.only(right: 20),
                                child: orderList.item2
                                    ? const HeroIcon(
                                        HeroIcons.chevronUp,
                                      )
                                    : const HeroIcon(
                        ...
                        onTap: () async {
                          tokenExpireWrapper(ref, () async {
                            ...
                            if (!loaded) {
                              final newList = await orderListNotifier
