...
class MockListNotifier extends ListNotifier<MockData> {
  ...
  Future<AsyncValue<List<MockData>>> testLoadList(
      Future<List<MockData>> Function() f) async {
    return loadList(f);
  }
  ...
  group('Testing ListNotifier : add', () {
    test('Should returns true and updates state on success', () async {
      ...
      expect(
          notifier.state.when(
              data: (d) => d,
              error: (e, s) => List<MockData>.empty(),
              loading: () => List<MockData>.empty()),
          newDataList);
    });
    ...
    test(
      ...
      try {
        ...
        expect(notifier.state, isA<AsyncData<List<MockData>>>()); // not reached
      } catch (e) {
        expect(e, error);
        expect(notifier.state, isA<AsyncData<List<MockData>>>());
        expect(
