...
import 'package:myecl/greencode/providers/all_greencode_item_list_provider.dart';
...
final greencodeItems = [
  ...
  GreenCodeItem.empty().copyWith(
    ...
    users: [
      User.empty().copyWith(id: '1'),
      User.empty().copyWith(id: '2'),
    ],
  ),
];
void main() {
  group('GreenCodeItemListNotifier', () {
    late AllGreenCodeItemListNotifier greencodeItemListNotifier;

    setUp(() {
      greencodeItemListNotifier = AllGreenCodeItemListNotifier(
    ...
    test('addGreenCodeItem', () async {
      ...
      when(
        () => greencodeItemListNotifier.greencodeItemRepository
            .addGreenCodeItem(newGreenCodeItem),
      ).thenAnswer((_) async => newGreenCodeItem);
      greencodeItemListNotifier.state =
          AsyncValue.data(greencodeItems.sublist(0));
      await greencodeItemListNotifier.addGreenCodeItem(newGreenCodeItem);
    ...
    test('updateGreenCodeItem', () async {
      final updatedGreenCodeItem = GreenCodeItem.empty().copyWith(
        id: '2',
        qrCodeContent: 'QRCodeContent 2',
        title: 'GreenCodeItem 2',
        content: 'GreenCodeItem 2 content',
      ...
      when(
        () => greencodeItemListNotifier.greencodeItemRepository
            .updateGreenCodeItem(updatedGreenCodeItem),
      ).thenAnswer((_) async => true);

      greencodeItemListNotifier.state =
          AsyncValue.data(greencodeItems.sublist(0));
      await greencodeItemListNotifier.updateGreenCodeItem(updatedGreenCodeItem);
      ...
      expect(
        ...
        greencodeItems
            .map(
              (item) => item.id == updatedGreenCodeItem.id
                  ? updatedGreenCodeItem
                  : item,
            )
            .toList(),
      );
    ...
    test('deleteGreenCodeItem', () async {
      final deletedGreenCodeItem = greencodeItems[1];
      when(
        () => greencodeItemListNotifier.greencodeItemRepository
            .deleteGreenCodeItem(deletedGreenCodeItem.id),
      ).thenAnswer((_) async => true);

      greencodeItemListNotifier.state =
          AsyncValue.data(greencodeItems.sublist(0));
      await greencodeItemListNotifier.deleteGreenCodeItem(deletedGreenCodeItem);
